...
ai-ass-core построен по принципу **микросервисной архитектуры** с использованием Docker контейнеров. Все сервисы объединены в единую Docker-сеть `ai-network` и взаимодействуют через внутренние DNS-имена.
...
3. **Single responsibility** — каждый сервис отвечает за одну функцию
4. **Self-hosted** — все компоненты работают локально без внешних зависимостей
5. **GPU-first** — оптимизация для GPU inference с fallback на CPU
...
- **Модели**:
  - LLM: `qwen3:8b`, `llama3:8b`, `mistral:7b`
